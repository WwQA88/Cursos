...
public class UserJsonTest {
	...
	public void deveVerificarListaNaRaiz() {
		...
		.then()
			...
			.body("name", hasItems("Jo�o da Silva","Maria Joaquina","Ana J�lia")) // Getting all attributes "name" in the array/list
	...
	public void devoFazerVerificacoesAvancadas() {
		...
		.then()
			...
			.body("findAll{it.age <= 25}[-1].name",is("Ana J�lia")) // Returning the last data
			.body("find{it.age <= 25}.name",is("Maria Joaquina")) // Returning the first data of the array/list
			.body("findAll{it.name.contains('n')}.name",hasItems("Maria Joaquina", "Ana J�lia")) // Verifying if has 'n' in the String
			.body("findAll{it.name.length() > 10}.name",hasItems("Maria Joaquina", "Jo�o da Silva")) // Verifying if has more than 10 characters in the String
